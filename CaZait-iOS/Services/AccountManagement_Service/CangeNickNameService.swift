...
class ChangeNickNameService {
    ...
    func getChangeNickNameInfo(nickName : String, completion : @escaping (NetworkResult<Any>) -> Void) {
        
        let userId = UserDefaults.standard.string(forKey: "userId")!
        let url = "\(APIConstants.nickNameChangeURL)/\(userId)"
        ...
        dataRequest.responseData{ dataResponse in
            ...
            case .success:
                ...
                case .tokenErr: //토큰 에러가 났을 경우 토큰 재발급 후, 다시 같은 통신을 재귀적으로 호출
                    self.refreshTokenAndRetryRequest(nickName: nickName, completion: completion)
                default:
                    completion(networkResult)
                }
    ...
    private func isVaildData(data: Data) -> NetworkResult<Any> {
        ...
        guard let decodedData = try? decoder.decode(ChangeNickNameResponse.self, from: data) else { return .pathErr }
    ...
    //토큰 재발급 함수
    private func refreshTokenAndRetryRequest(nickName: String, completion: @escaping (NetworkResult<Any>) -> Void) {
        RefreshTokenService.shared.getRefreshToken() { response in
            ...
            case .success(let data):
                ...
                self.getChangeNickNameInfo(nickName: nickName, completion: completion)
