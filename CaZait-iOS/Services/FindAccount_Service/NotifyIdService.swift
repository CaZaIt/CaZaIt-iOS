...
class NotifyIdService {
    ...
    static let shared = NotifyIdService()
    ...
    func getIdInfo(phoneNumber: String, completion : @escaping (NetworkResult<Any>) -> Void) {
        
        let url = "\(APIConstants.notifyIdURL)"
        let header : HTTPHeaders = ["Content-Type" : "application/json"]
        
        let parameters: [String: Any] = ["userPhoneNumber": phoneNumber]
    ...
    private func isVaildData(data: Data) -> NetworkResult<Any> {
        ...
        guard let decodedData = try? decoder.decode(NotifyIdResponse.self, from: data) else { return .pathErr }
