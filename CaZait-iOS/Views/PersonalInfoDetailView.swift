...
class PersonalInfoDetailTermsView: UIViewController, UIGestureRecognizerDelegate{
    ...
    override func viewDidLoad() {
        ...
        // Set preferredMaxLayoutWidth to enable multiline text layout in the label.
        label.preferredMaxLayoutWidth = UIScreen.main.bounds.width - 16 // Set appropriate width to prevent the text from wrapping infinitely.
        
        // UILabel을 스크롤 가능한 scrollView의 서브뷰로 추가합니다.
        scrollView.addSubview(label)
        
        // UILabel의 크기를 UILabel의 컨텐츠 크기에 따라 동적으로 조정하기 위해 아래와 같이 설정합니다.
    ...
    @objc func backButtonTapped() {
        if let navigationController = self.navigationController {
            ...
            // and the back button is tapped, simply dismiss the current view controller.
